...
import type { Result, Teacher, Subject, Employee, Student, Courses } from "@/models/types";
...
const App: React.FC<{ coursesFauts: any; loadingFaults: boolean }> = ({
    coursesFauts,
    loadingFaults,
}) => {
  ...
  const faults = coursesFauts?.result?.data as Item[];
  const result = coursesFauts?.result as Result;

  useEffect(() => {
    if (result?.count > 0) {
      const dataWithKeys = faults.map((row, index) => {
        let rowData = {};
